"""Tests for ASAM ODS URL path resolution (_file_path_resolver.py)."""
...
from wodson.utils._file_path_resolver import (
    FilePathResolver,
...
FILE_MODE_ABSOLUTE = FilePathResolver.FILE_MODE_ABSOLUTE
FILE_MODE_SINGLE_VOLUME = FilePathResolver.FILE_MODE_SINGLE_VOLUME
FILE_MODE_MULTI_VOLUME = FilePathResolver.FILE_MODE_MULTI_VOLUME

FILE_NOTATION_UNC_WIN = FilePathResolver.FILE_NOTATION_UNC_WIN
FILE_NOTATION_UNC_UNIX = FilePathResolver.FILE_NOTATION_UNC_UNIX
FILE_NOTATION_URL = FilePathResolver.FILE_NOTATION_URL

JOIN_BEHAVIOR_DEFAULT: Literal["DEFAULT"] = cast(
    Literal["DEFAULT"],
    FilePathResolver.JOIN_BEHAVIOR_DEFAULT,
)
JOIN_BEHAVIOR_ATTACH: Literal["ATTACH"] = cast(
    Literal["ATTACH"],
    FilePathResolver.JOIN_BEHAVIOR_ATTACH,
)

SOURCE_ATTR_FILENAME = FilePathResolver.SOURCE_ATTR_FILENAME
SOURCE_ATTR_FLAGS_FILENAME = FilePathResolver.SOURCE_ATTR_FLAGS_FILENAME
SOURCE_ATTR_LOCATION = FilePathResolver.SOURCE_ATTR_LOCATION
SOURCE_ATTR_AO_LOCATION = FilePathResolver.SOURCE_ATTR_AO_LOCATION

CTX_FILE_MODE = FilePathResolver.CTX_FILE_MODE
CTX_FILE_NOTATION = FilePathResolver.CTX_FILE_NOTATION
CTX_FILE_SYMBOLS = FilePathResolver.CTX_FILE_SYMBOLS
CTX_FILE_ROOT = FilePathResolver.CTX_FILE_ROOT
CTX_FILE_ROOT_EXTREF = FilePathResolver.CTX_FILE_ROOT_EXTREF
CTX_FILE_ROOT_MANAGED = FilePathResolver.CTX_FILE_ROOT_MANAGED
...
# ============================================================================
# Tests: FilePathResolver._get_attribute_path_mode
# ============================================================================
...
def test_get_source_attr_from_extref(mock_ods_attribute):
    ...
    assert FilePathResolver._get_attribute_path_mode(attr) is FilePathResolver.AttrMode.EXTREF
...
def test_get_source_attr_from_filename_url(mock_ods_attribute):
    ...
    assert FilePathResolver._get_attribute_path_mode(attr) is FilePathResolver.AttrMode.ROOT
...
def test_get_source_attr_from_ao_location(mock_ods_attribute):
    ...
    assert FilePathResolver._get_attribute_path_mode(attr) is FilePathResolver.AttrMode.MANAGED
...
def test_get_source_attr_unknown_raises(mock_ods_attribute):
    ...
    with pytest.raises(UnknownSourceAttributeError, match="not a recognized file attribute"):
        FilePathResolver._get_attribute_path_mode(attr)
...
def test_absolute_mode_windows_path():
    ...
    result = FilePathResolver(ctx).resolve_url(r"C:\data\file.dat", _make_attribute(SOURCE_ATTR_FILENAME))
...
def test_absolute_mode_unix_path():
    ...
    result = FilePathResolver(ctx).resolve_url("/data/file.dat", _make_attribute(SOURCE_ATTR_FILENAME))
...
def test_absolute_mode_unc_path():
    ...
    ctx = {CTX_FILE_MODE: FILE_MODE_ABSOLUTE, CTX_FILE_NOTATION: FILE_NOTATION_UNC_WIN}
    result = FilePathResolver(ctx).resolve_url(r"\\server\share\file.dat", _make_attribute(SOURCE_ATTR_FILENAME))
...
def test_single_volume_filename_url_windows():
    ...
    result = FilePathResolver(ctx).resolve_url(r"measurements\file.dat", _make_attribute(SOURCE_ATTR_FILENAME))
...
def test_single_volume_filename_url_unix():
    ...
    result = FilePathResolver(ctx).resolve_url("measurements/file.dat", _make_attribute(SOURCE_ATTR_FILENAME))
...
def test_single_volume_flags_filename_url_windows():
    ...
    result = FilePathResolver(ctx).resolve_url(r"flags\file.btf", _make_attribute(SOURCE_ATTR_FLAGS_FILENAME))
...
def test_single_volume_location_windows():
    ...
    result = FilePathResolver(ctx).resolve_url(r"refs\doc.pdf", _make_attribute(SOURCE_ATTR_LOCATION))
...
def test_single_volume_ao_location_with_managed_windows():
    ...
    result = FilePathResolver(ctx).resolve_url(r"ao\file.dat", _make_attribute(SOURCE_ATTR_AO_LOCATION))
...
def test_single_volume_resolve_url_accepts_source_mode_root_windows():
    """resolve_url accepts FilePathResolver.AttrMode.ROOT directly."""
    ctx = {
        CTX_FILE_MODE: FILE_MODE_SINGLE_VOLUME,
        CTX_FILE_NOTATION: FILE_NOTATION_UNC_WIN,
        CTX_FILE_ROOT: r"C:\data",
    }
    result = FilePathResolver(ctx).resolve_url(r"measurements\file.dat", FilePathResolver.AttrMode.ROOT)
    assert result == r"C:\data\measurements\file.dat"


@pytest.mark.skipif(platform.system() != "Windows", reason="Windows-specific test")
def test_single_volume_resolve_url_accepts_source_mode_extref_windows():
    """resolve_url accepts FilePathResolver.AttrMode.EXTREF directly."""
    ctx = {
        CTX_FILE_MODE: FILE_MODE_SINGLE_VOLUME,
        CTX_FILE_NOTATION: FILE_NOTATION_UNC_WIN,
        CTX_FILE_ROOT_EXTREF: r"C:\extref",
    }
    result = FilePathResolver(ctx).resolve_url(r"refs\doc.pdf", FilePathResolver.AttrMode.EXTREF)
    assert result == r"C:\extref\refs\doc.pdf"


@pytest.mark.skipif(platform.system() != "Windows", reason="Windows-specific test")
def test_single_volume_resolve_url_accepts_source_mode_managed_windows():
    """resolve_url accepts FilePathResolver.AttrMode.MANAGED directly."""
    ctx = {
        CTX_FILE_MODE: FILE_MODE_SINGLE_VOLUME,
        CTX_FILE_NOTATION: FILE_NOTATION_UNC_WIN,
        CTX_FILE_ROOT: r"C:\data",
    ...
    result = FilePathResolver(ctx).resolve_url(r"ao\file.dat", FilePathResolver.AttrMode.MANAGED)
    assert result == r"C:\managed\ao\file.dat"
...
def test_single_volume_missing_root_raises():
    ...
    with pytest.raises(MissingContextVariableError, match="FILE_ROOT"):
        FilePathResolver(ctx).resolve_url(r"file.dat", _make_attribute(SOURCE_ATTR_FILENAME))
...
def test_single_volume_missing_extref_root_raises():
    ...
    with pytest.raises(MissingContextVariableError, match="FILE_ROOT_EXTREF"):
        FilePathResolver(ctx).resolve_url(r"refs\doc.pdf", _make_attribute(SOURCE_ATTR_LOCATION))
...
def test_multi_volume_with_symbol_windows():
    ...
    result = FilePathResolver(ctx).resolve_url(r"$(ROOT)measurements\file.dat", _make_attribute(SOURCE_ATTR_FILENAME))
...
def test_multi_volume_with_symbol_unix():
    ...
    result = FilePathResolver(ctx).resolve_url("$(ROOT)measurements/file.dat", _make_attribute(SOURCE_ATTR_FILENAME))
...
def test_multi_volume_multiple_symbols_windows():
    ...
    result = FilePathResolver(ctx).resolve_url(r"$(BACKUP)archive\file.dat", _make_attribute(SOURCE_ATTR_FILENAME))
...
def test_multi_volume_no_symbol_returns_unchanged():
    ...
    result = FilePathResolver(ctx).resolve_url(r"C:\absolute\file.dat", _make_attribute(SOURCE_ATTR_FILENAME))
...
def test_multi_volume_unknown_symbol_raises():
    ...
    with pytest.raises(UnknownSymbolError, match="Unknown symbol 'UNKNOWN'"):
        FilePathResolver(ctx).resolve_url(r"$(UNKNOWN)file.dat", _make_attribute(SOURCE_ATTR_FILENAME))
...
def test_multi_volume_symbol_lookup_is_case_insensitive_and_trims_symbols():
    ...
    result = FilePathResolver(ctx).resolve_url(r"$(RoOt)folder\file.dat", _make_attribute(SOURCE_ATTR_FILENAME))
...
def test_multi_volume_missing_closing_paren_raises():
    ...
    with pytest.raises(InvalidMultiVolumePathError, match="missing closing paren"):
        FilePathResolver(ctx).resolve_url(r"$(ROOTfile.dat", _make_attribute(SOURCE_ATTR_FILENAME))
...
def test_multi_volume_symbol_not_in_context_raises():
    ...
    with pytest.raises(MissingContextVariableError, match="not defined in context"):
        FilePathResolver(ctx).resolve_url(r"$(ROOT)file.dat", _make_attribute(SOURCE_ATTR_FILENAME))
...
def test_normalize_url_unc_path():
    ...
    result = FilePathResolver(ctx).resolve_url("//server/share/file.dat", _make_attribute(SOURCE_ATTR_FILENAME))
    assert result.startswith("file://")
...
def test_missing_file_mode_raises():
    ...
    with pytest.raises(MissingContextVariableError, match="FILE_MODE"):
        FilePathResolver(ctx).resolve_url("file.dat", _make_attribute(SOURCE_ATTR_FILENAME))
...
def test_invalid_source_attr_raises():
    ...
    attr.name = "test"
    with pytest.raises(UnknownSourceAttributeError, match="not a recognized file attribute"):
        FilePathResolver(ctx).resolve_url("file.dat", attr)
...
def test_non_dict_context_raises_type_error() -> None:
    """FilePathResolver only accepts plain dict context values."""
    ...
    with pytest.raises(TypeError, match="dict\\[str, str\\]"):
        FilePathResolver(cast(dict[str, str], MockContext()))


def test_invalid_join_behavior_raises_value_error() -> None:
    """FilePathResolver rejects unsupported join_behavior values."""
...
# ============================================================================
# Tests: FilePathResolver class
# ============================================================================


@pytest.mark.skipif(platform.system() != "Windows", reason="Windows-specific test")
def test_file_path_resolver_class_single_volume():
    """FilePathResolver class resolves paths correctly."""
    ctx = {
        CTX_FILE_MODE: FILE_MODE_SINGLE_VOLUME,
        CTX_FILE_NOTATION: FILE_NOTATION_UNC_WIN,
        CTX_FILE_ROOT: r"C:\data",
...
def test_file_path_resolver_class_absolute():
    """FilePathResolver class handles ABSOLUTE mode."""
    ctx = {CTX_FILE_MODE: FILE_MODE_ABSOLUTE, CTX_FILE_NOTATION: FILE_NOTATION_UNC_WIN}
    resolver = FilePathResolver(ctx)
...
@pytest.mark.skipif(platform.system() != "Windows", reason="Windows-specific test")
def test_file_path_resolver_class_multi_volume():
    """FilePathResolver class handles MULTI_VOLUME mode."""
    ctx = {
        CTX_FILE_MODE: FILE_MODE_MULTI_VOLUME,
        CTX_FILE_NOTATION: FILE_NOTATION_UNC_WIN,
...
def test_file_path_resolver_case_insensitive_context():
    """FilePathResolver handles case-insensitive keys, values, and source attrs."""
    ctx = {
        "file_mode": "absolute",
...
def test_resolve_urls_absolute_mode(mock_ods_attribute):
    """FilePathResolver.resolve_urls resolves multiple URLs in ABSOLUTE mode."""
    attr = mock_ods_attribute(base_name=SOURCE_ATTR_FILENAME)
    ctx = {CTX_FILE_MODE: FILE_MODE_ABSOLUTE, CTX_FILE_NOTATION: FILE_NOTATION_UNC_WIN}
    resolver = FilePathResolver(ctx)
...
def test_resolve_urls_single_volume(mock_ods_attribute):
    """FilePathResolver.resolve_urls resolves multiple URLs in SINGLE_VOLUME mode."""
    attr = mock_ods_attribute(base_name=SOURCE_ATTR_FILENAME)
    ctx = {
        CTX_FILE_MODE: FILE_MODE_SINGLE_VOLUME,
        CTX_FILE_NOTATION: FILE_NOTATION_UNC_WIN,
...
def test_resolve_urls_empty_list(mock_ods_attribute):
    """FilePathResolver.resolve_urls handles empty list."""
    attr = mock_ods_attribute(base_name=SOURCE_ATTR_FILENAME)
    ctx = {CTX_FILE_MODE: FILE_MODE_ABSOLUTE, CTX_FILE_NOTATION: FILE_NOTATION_UNC_WIN}
    resolver = FilePathResolver(ctx)
...
def test_resolve_urls_with_file_path_resolver_class(mock_ods_attribute):
    """FilePathResolver.resolve_urls works correctly."""
    attr = mock_ods_attribute(base_name=SOURCE_ATTR_FILENAME)
    ctx = {CTX_FILE_MODE: FILE_MODE_ABSOLUTE, CTX_FILE_NOTATION: FILE_NOTATION_UNC_WIN}
    resolver = FilePathResolver(ctx)
...
def test_resolve_urls_accepts_source_mode_directly():
    ...
    results = resolver.resolve_urls(urls, FilePathResolver.AttrMode.ROOT)
    assert results == [
        r"C:\data\file1.dat",
        r"C:\data\subdir\file2.dat",
    ]
