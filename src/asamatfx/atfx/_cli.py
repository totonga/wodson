"""CLI commands for the ``atfx`` submodule.
...
def register_atfx_subparser(parent_sub: argparse._SubParsersAction) -> None:  # type: ignore[type-arg]
    """Register all ``atfx`` subcommands onto *parent_sub*."""
    atfx = parent_sub.add_parser(
        "atfx",
        help="Commands for ATFX files.",
    )
    sub = atfx.add_subparsers(dest="command", required=True)
...
def dispatch(args: argparse.Namespace) -> None:
    """Dispatch an already-parsed *args* namespace to the right atfx handler."""
